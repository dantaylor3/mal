...
export const globalEnv: Env = createFromMap({
  ...
  'symbol?': Function((...args) => Boolean(args[0].t === 'symbol')),
  count: Function((...args) => {
    switch (args[0].t) {
      case 'list':
  ...
  not: Function((...args) => {
    assertCozyType('boolean', args[0])
    return Boolean(!args[0].v)
  }),
  '<': Function((...args) => {
    const [a, b] = args
  ...
  map: Function((...args) => {
    ...
    return List(list.v.map(e => f.v(e)))
  }),
})
