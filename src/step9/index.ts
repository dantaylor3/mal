...
import {CozyType, Symbol, List, assertCozyType, Null, Function, assertCozyArray, String, isCozyType} from './types'
    ...
    case 'list': {
      ...
      if (ast.v[0].t === 'symbol') {
        switch (ast.v[0].v) {
          ...
          case 'try*': {
            const [, tryBody, catchList] = ast.v
            assertCozyType('list', catchList)
            const [catchSymbol, e, catchBody] = catchList.v
            assertCozyType('symbol', e)
            assert.ok(catchSymbol.v === 'catch*')

            try {
              return evalCozy(tryBody, env)
            } catch (ex) {
              return evalCozy(
                catchBody,
                create([e], [isCozyType(ex) ? ex : String((ex as Error).message || 'unknown error')], env)
              )
            }
          }
        }
